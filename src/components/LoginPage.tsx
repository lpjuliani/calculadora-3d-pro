...
const LoginPage: React.FC = () => {
  ...
  // Mostrar credenciais padrão em produção se não houver usuários
  React.useEffect(() => {
    const hasUsers = Object.keys(authState.users).length > 0;
    const isProduction = typeof window !== 'undefined' && window.location.hostname !== 'localhost';
    setShowDefaultCredentials(isProduction && !hasUsers);
  }, [authState.users]);
        ...
        <div className="text-center mt-8">
          {showDefaultCredentials && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <p className="text-blue-800 font-medium mb-2">Credenciais padrão:</p>
              <p className="text-blue-700 text-sm">
                <strong>Usuário:</strong> admin<br />
                <strong>Senha:</strong> admin2024
              </p>
            </div>
          )}
