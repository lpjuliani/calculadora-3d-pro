...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "ADD_ACCESSORY": return { ...state, accessories: [...state.accessories, action.payload] };
    ...
    case "ADD_PACKAGING": return { ...state, packaging: [...state.packaging, action.payload] };
  ...
  useEffect(() => {
    (async () => {
      if (auth.isAuthenticated && auth.currentUser?.email) {
        const userId = await ensureProfileId(auth.currentUser.email, auth.currentUser.role);
        const data = await loadAllForUser(userId);
        dispatch({ type: "LOAD_DATA", payload: data });
      } else {
        dispatch({ type: "RESET_DATA" });
      }
    })();
  }, [auth.isAuthenticated, auth.currentUser?.email, auth.currentUser?.role]);
